...
async def plan(client, message):
    ...
    btn = [[
            ...
            InlineKeyboardButton('•🎁 ꜰʀᴇᴇ 5-ᴍɪɴ ᴘʀᴇᴍɪᴜᴍ •', callback_data='give_trial')
        ],[
    ...
    try:
        ...
        if amount in STAR_PREMIUM_PLANS:
            ...
            if seconds > 0:
                ...
                # Bug fix: `message.user` doesn't exist in Pyrogram (it's
                # `message.from_user`) — this used to throw AttributeError
                # every time, silently swallowed by the outer except below,
                # so the admin log for star payments never actually sent.
